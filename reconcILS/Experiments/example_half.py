...
for ili in range(len(sort_)):
                ...
                if len(li)==0:
                        ...
                        df = pd.DataFrame(dic)
                        df.to_csv('bio_result_ZeroCol_ASTRAL_ML_ALLPARALOGS_MIN27_pruned_half_1.csv', index=False)
                        dic1= {'time':time_it}
                        df_time= pd.DataFrame(dic1)
                        df_time.to_csv('timing_ZeroCol_ASTRAL_ML_ALLPARALOGS_MIN27_pruned_half_1.csv', index=False)
                        oer= open('labeled_L_ZeroCol_ASTRAL_ML_ALLPARALOGS_MIN27_pruned_half_1.csv','w+')
                        ...
                        dic1= {'time':time_it2}
                        df_time= pd.DataFrame(dic1)
                        df_time.to_csv('timing_ZeroCol_ASTRAL_ML_ALLPARALOGS_MIN27_intro_1_write_pruned_half_1.csv', index=False)


                        dic_prune_G ={'Repliccate': replicate_prune_G,'Gene_trees':gene_prune_G}
    
                        df_prune_G=  pd.DataFrame(dic_prune_G)
                        df_prune_G.to_csv('ZeroCol_ASTRAL_ML_ALLPARALOGS_MIN27_pruned_Gene_trees_half_1.csv', index=False)
