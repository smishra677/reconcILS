...
def prune_trees(newick):
        ...
        t= ete3.Tree(newick)
        list_to_prune1=[]
        for node in species_to_letters:
                if t.search_nodes(name=node):
                        list_to_prune1.append(node)
        ...
        t.prune(list(set(list_to_prune1)))
...
for ili in range(len(sort_)):
                ...
                if len(li)==0:
                        ...
                        df = pd.DataFrame(dic)
                        df.to_csv('bio_result_ZeroCol_ASTRAL_ML_SCO_MIN27_pruned_fourth.csv', index=False)
                        dic1= {'time':time_it}
                        df_time= pd.DataFrame(dic1)
                        df_time.to_csv('timing_ZeroCol_ASTRAL_ML_SCO_MIN27_pruned_fourth.csv', index=False)
                        oer= open('labeled_L_ZeroCol_ASTRAL_ML_SCO_MIN27_pruned_fourth.csv','w+')
                        ...
                        dic1= {'time':time_it2}
                        df_time= pd.DataFrame(dic1)
                        df_time.to_csv('timing_ZeroCol_ASTRAL_ML_SCO_MIN27_intro_1_write_pruned_fourth.csv', index=False)


                        dic_prune_G ={'Repliccate': replicate_prune_G,'Gene_trees':gene_prune_G}
    
                        df_prune_G=  pd.DataFrame(dic_prune_G)
                        df_prune_G.to_csv('ZeroCol_ASTRAL_ML_SCO_MIN27_pruned_Gene_trees_fourth.csv', index=False)
