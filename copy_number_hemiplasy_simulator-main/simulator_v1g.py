...
def count_dc(gene_tree, sp_tree):
    ...
    for edge in sp_tree.postorder_edge_iter():
        ...
        # get relevant time frame and taxa for species tree edge
        if edge.length == None:
            relevant_time_frame = [edge.head_node.age, np.inf]
        else:
        ...
        # find gene tree edges within relevant time frame and with correct taxa
        for gtedge in gene_tree.postorder_edge_iter():
            gt_relevant_time_frame = [gtedge.head_node.age, gtedge.head_node.age + gtedge.length]
            gt_relevant_leaves = get_leaves(gtedge)
            gt_relevant_leaves = [x.split()[0] for x in gt_relevant_leaves]
            if gt_relevant_time_frame[0] < relevant_time_frame[1] and gt_relevant_time_frame[1] > relevant_time_frame[1] and len(intersection(gt_relevant_leaves, relevant_leaves))>0:
                count_relevant += 1
        
        # if more than one branch meets these criteria, we have deep coalescense
        if count_relevant > 1:
            dc_count+=count_relevant-1
    return(dc_count)
                
def count_dc_daughter(gene_tree, sp_tree):
    ...
    #print(species_subtree)

    dc_count = 0
    for edge in species_subtree.postorder_edge_iter():
...
def birth_death(sp_tree, lambda_par, mu_par):
    ...
    ils_1 += count_dc(gene_tree=parent_tree, sp_tree=sp_tree)
    #print('This is my custom count', ils_1)
        ...
        while tc < edge.length and N > 0:
            ...
            if ti + tc > edge.length:
                ...
                if event_prob < (lambda_par / (lambda_par+mu_par)):
                    ...
                    ils_3 += daughter_dc
                    #print("This is my daughter DC count: ", daughter_dc)
